			...
			<table id="table-2">
				...
				<c:forEach var="omVO" items="${list}" >		 
							<c:choose>
									...
									<c:when test="${(((omVO.payID == 1) and omVO.payStatus == 1) and omVO.trfStatus == 0) 
									and ( ((omVO.ordStatus == 9) and ((omVO.shipStatus == 0 or omVO.shipStatus == 3 ))))}">
							<tr>
								<td><input  type="checkbox" name="c" value="${omVO.ordID}" /></td>
								<td>${memSVC.getOneMember(omVO.rentID).accountName}</td>
								<td>${omVO.ordPrice}</td>
								<td>${memSVC.getOneMember(omVO.rentID).bankCode}</td>
								<td>${memSVC.getOneMember(omVO.rentID).bankAccount}</td>
								<td>${memSVC.getOneMember(omVO.rentID).idcn}</td>
								<td>${omVO.ordID}</td>
