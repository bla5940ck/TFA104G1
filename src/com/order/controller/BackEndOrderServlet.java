...
//						System.out.println("錯了嗎?????????????");
						RequestDispatcher failureView = req
								.getRequestDispatcher("/back_end/order/listAllOrderList.jsp");
						failureView.forward(req, res);
						return; // 程式中斷
					}
				}
...
//			System.out.println("進來了");
		...
		if ("get_date_manager_order".equals(action)) {
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					RequestDispatcher failureView = req.getRequestDispatcher("/back_end/order/listAllOrderMaster.jsp");
					failureView.forward(req, res);
					return;// 程式中斷
				}
...
//			Integer memID = (Integer) req.getSession().getAttribute("id");
				...
				req.getRequestDispatcher("/back_end/order/listAllOrderMaster.jsp").forward(req, res);
				return;
			} catch (Exception e) {
				e.printStackTrace();
				errorMsgs.add("修改資料失敗:" + e.getMessage());
		...
		if ("update_for_problem".equals(action)) {
			System.out.println("後台訂單更新");
			List<String> errorMsgs = new LinkedList<String>();
